<template >
  ...
  <div class="box">
    ...
    <li class="h-40">
      <button type="button" class="waves-effect waves-circle btn btn-circle btn-primary mb-5" @click="openModal('add-marche')"  ><i class="mdi mdi-plus"></i></button>
    </li>
...
<div class="box-body">
 ...
 <div class="modal center-modal fade" id="add-marche"  ref="add-marche" tabindex="-1">
  ...
  <div class="modal-content">
    ...
    <h5 class="modal-title">Ajouter un debarcadère</h5>
    ...
    <div class="row mt-3 content-group">
                      ...
                      <div class="col">
                        <div class="input-groupe">
                          <label for="userpassword"
                            > Jours du  debarcadère <span class="text-danger">*</span></label
                          >
                          <MazSelect
                            v-model="step1.jour_du_marche"
                            color="secondary"
                            name="step1.jour_du_marche"
                            size="sm"
                            rounded-size="sm"
                            search
                            :options="jours"
    ...
    <div class="modal-footer modal-footer-uniform text-end">
      ...
      <button type="button" @click="SubmitCollecteur('add-marche')" class="waves-effect waves-light btn btn-primary ">Valider</button>
        ...
        <div class="modal-content">
          <div class="modal-header">
          <h5 class="modal-title">Modifier un debarcadère</h5>
...
async fetchCollecteurs() {
  ...
  async SubmitCollecteur(modalId) {
   ...
   console.log('data',data)
    ...
    this.successmsg(
      "Création du debarcadère",
      "Votre debarcadère a été créé avec succès !"
    );
  ...
  if (this.v$.$errors.length == 0) {
    ...
    try {
      ...
      if (response.status === 200) {
        this.closeModal(modalId);
        this.successmsg(
      "Mise à jour du debarcadère",
      "Votre debarcadère a été mis à jour avec succès !"
    );
    this.$emit('point-collecte-updated');
    this.loading = false
     ...
     this.ConfirmeDelete(id);
       ...
       try {
         ...
         if (response.status === 200) {
           this.loading = false
     this.successmsg(
          "Suppression du debarcadère",
          "Votre debarcadère a été supprimé avec succès !"
        );
        this.$emit('point-collecte-updated');
        this.loading = false
