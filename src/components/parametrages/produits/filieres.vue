...
<div class="box">
   <div class="box-header with-border d-flex justify-content-between align-items-center p-3">
     <h3 class="box-title mb-0">Liste des Filières</h3>
         ...
         <table id="example1" class="table table-bordered table-striped">
           ...
           <tbody v-else>
               <tr v-for="(data )  in paginatedItems" :key="data.id">
                   ...
                   <td style="width:100px !important">{{ data?.code_filiere_produit }}</td>
                   <td>{{ data?.nom_filiere_produit }}</td>
                   
                   <td style="width: 100px;">
                    <div class="d-flex justify-content-evenly border-0">
                        <a href="javascript:void(0)" class="btn btn-circle btn-info btn-xs" title="" @click="HandleIdUpdate(data.code_filiere_produit ,'update-forme')"  data-original-title="Update" ><i class="ti-marker-alt"></i></a>
                        <a href="javascript:void(0)" class="btn btn-circle btn-danger btn-xs" @click="HandleIdDelete(data.id_filiere_produit)" title="" data-toggle="tooltip" data-original-title="Delete"><i class="ti-trash"></i></a>
                    </div>   
					</td > 
               </tr>
           </tbody>
	  ...
	  <div class="modal-dialog modal-lg">
		<div class="modal-content">
		  <div class="modal-header">
			<h5 class="modal-title">Ajouter des Filières</h5>
                ...
                <div class="row align-items-center p-2  border-bottom " v-for="(forme, index) in Formes" :key="forme.id">
                  ...
                  <div class="col">
                    <div class="input-groupe">
                      ...
                      >
                      <MazInput
                        v-model="forme.code_filiere_produit"
                        color="info"
                        name="code_filiere_produit"
                        size="sm"
                        rounded-size="sm"
                        type="text"
                        @input="clearErrorFormes(index, 'code_filiere_produit')"

                      />
                      <small v-if="errors.Formes && errors.Formes[index] && errors.Formes[index].code_filiere_produit">{{ errors.Formes[index].code_filiere_produit }}</small>
                  ...
                  <div class="col">
                    <div class="input-groupe">
                      ...
                      >
                      <MazInput
                        v-model="forme.nom_filiere_produit"
                        color="info"
                        name="nom_filiere_produit"
                        size="sm"
                        rounded-size="sm"
                        type="text"
                        @input="clearErrorFormes(index, 'nom_filiere_produit')"

                      />
                      <small v-if="errors.Formes && errors.Formes[index] && errors.Formes[index].nom_filiere_produit">{{ errors.Formes[index].nom_filiere_produit }}</small>
                      <small v-if="resultError['Formes']"> {{ resultError["Formes"] }} </small>
                    </div>
                  </div>
                </div>
        ...
        <button type="button" @click="SubmitForme('add-forme')" class="waves-effect waves-light btn btn-primary ">Valider</button>
          ...
          <div class="modal-content">
            <div class="modal-header">
            <h5 class="modal-title">Modifier une filière</h5>
                  ...
                  <div class="row mt-3 content-group">
                        ...
                        <div class="col-12">
                          <div class="input-groupe">
                            ...
                            >
                            <MazInput
                              v-model="step2.code_filiere_produit"
                              color="secondary"
                              name="step2.code_filiere_produit"
                              size="sm"
                              rounded-size="sm"
                              type="text"
                            ...
                            />
                            <small v-if="v$.step2.code_filiere_produit.$error">{{
                              v$.step2.code_filiere_produit.$errors[0].$message
                            }}</small>
                        ...
                        <div class="col-12">
                          <div class="input-groupe">
                            ...
                            >
                            <MazInput
                              v-model="step2.nom_filiere_produit"
                              color="secondary"
                              name="step2.nom_filiere_produit"
                              size="sm"
                              rounded-size="sm"
                              type="text"
                            ...
                            />
                            <small v-if="v$.step2.nom_filiere_produit.$error">{{
                              v$.step2.nom_filiere_produit.$errors[0].$message
                            }}</small>
...
export default {
  ...
  async  mounted() {
    ...
    methods: {
        ...
        AddformDataFormes() {
     this.Formes.push({code_filiere_produit:"", nom_filiere_produit:"", });
   },
  ...
  clearErrorFormes(index, field) {   
   ...
   validateFormes() {
    ...
    this.Formes.forEach((forme, index) => {
        ...
        if (!forme.code_filiere_produit) {
            formeErrors.code_filiere_produit = 'Ce champs est obligatoire!';
            isValid = false;
        }
        if (!forme.nom_filiere_produit) {
            formeErrors.nom_filiere_produit = 'Ce champs est obligatoire!';
            isValid = false;
        }
    ...
    return isValid;
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          this.$emit('filiere-count', this.data.length)
    ...
    async SubmitForme(modalId) {
      ...
      if (this.validateFormes()) {
        ...
        this.loading = true;
        try {
          const response = await axios.post("/parametrages/filieres", this.Formes, {
       ...
       console.log('qfs',response)
          ...
          if (response.status === 200) {
     this.Formes = [{code_filiere_produit:"", nom_filiere_produit:"", }];

             this.closeModal(modalId);
             this.successmsg(
                "Création de filières",
                "Vos filières ont été créées avec succès !"
            );
            await this.fetchFormes();
          } else {
          }
    ...
    this.loading = true;
      ...
      try {
        const response = await axios.get(`/parametrages/filieres/${id}`, {
          headers: {
            Authorization: `Bearer ${this.loggedInUser.token}`
          }
        });
        ...
        if (response.status === 200) {
          ...
          this.step2.code_filiere_produit = data.code_filiere_produit,
          this.step2.nom_filiere_produit = data.nom_filiere_produit,
          
       
          this.ToId = data.id_filiere_produit
          this.loading = false;
   ...
   this.v$.step2.$touch();
    ...
    if (this.v$.$errors.length == 0) {
       ...
       this.loading = true;
       let data = {
            code_filiere_produit:this.step2.code_filiere_produit,
            nom_filiere_produit:this.step2.nom_filiere_produit,
            }

      try {
        const response = await axios.put(`/parametrages/filieres/${this.ToId}`,data, {
          headers: {
        ...
        if (response.status === 200) {
          ...
          this.successmsg(
                ...
                "Votre filière a été mise à jour avec succès !"
            );
            await this.fetchFormes();
     ...
     if (result.isConfirmed) {
       this.ConfirmeDelete(id);
         ...
         try {
           ...
           const response = await axios.delete(`/parametrages/filieres/${id}`, {
             headers: {
               Authorization: `Bearer ${this.loggedInUser.token}`,
             },
           ...
           if (response.status === 200) {
             this.loading = false
                ...
                "Votre filière a été supprimée avec succès !"
            );
            await this.fetchFormes();
...
if (this.searchForme !== null) {
  ...
  this.FormesOptions =this.data.filter(user => {
    const Nom = user.code_filiere_produit || '';
    const definition = user.nom_filiere_produit || '';
