   ...
   <div class="box-header with-border d-flex justify-content-between align-items-center p-3">
     ...
     <div class="navbar-custom-menu r-side col-4">
        ...
        <MazSelect v-model="year" color="secondary" secondary :options="yearOptions" search
			...
			<li class="h-40 me-3">
				<button type="button" class="waves-effect waves-circle btn btn-circle btn-primary mb-5"   @click="openModal('add-fiche-collecte')"   ><i class="mdi mdi-filter"></i></button>
			</li>
            <li class="h-40">
        ...
        :checked="data.enquete?.etat === true" 
         @change="updateSelection(data.enquete?.num_fiche, $event.target.checked)"
                 ...
                 >
                   ...
                   <td style="width:150px !important">{{ data?.enquete?.num_fiche }}</td>
                   <td>
                   ...
                   <td style="width: 120px;">
                    <div class="d-flex justify-content-evenly border-0">
                        ...
                        <router-link  v-if="parseInt(collecteData.id) === 1" :to="{ name: 'enquete-prix-par-marche-collecte', params: { id: data.enquete?.id , nom:'COLLECTE' }}"  class="btn btn-circle btn-success btn-xs"   title=""   @click="HandleData(data?.enquete)" data-original-title="view"> <i class="ti-eye"></i> </router-link>
                        <router-link  v-if="parseInt(collecteData.id) === 3"  :to="{ name: 'enquete-prix-par-marche-hebdomadaire', params: { id: data.enquete?.id , nom:'CONSOMMATION' }}" class="btn btn-circle btn-success btn-xs"  title=""  @click="HandleData(data?.enquete)" data-original-title="view" ><i class="ti-eye"></i></router-link>
                        <router-link  v-if="parseInt(collecteData.id) === 4"  :to="{ name: 'enquete-prix-par-marche-journaliere', params: { id: data.enquete?.id , nom:'JOURNALIERE' }}" class="btn btn-circle btn-success btn-xs" title="" @click="HandleData(data?.enquete)" data-original-title="view"><i class="ti-eye"></i> </router-link>
                        <router-link  v-if="parseInt(collecteData.id) === 8" :to="{ name: 'enquete-prix-par-marche-transfrontalier', params: { id: data.enquete?.id , nom:'TRANSFRONTALIER' }}" class="btn btn-circle btn-success btn-xs" title="" @click="HandleData(data?.enquete)"  data-original-title="view" ><i class="ti-eye"></i></router-link>


                        <a  v-if="parseInt(collecteData.id) === 1" href="javascript:vcode(0)" class="btn btn-circle btn-danger btn-xs" @click="HandleIdDelete(data.enquete?.id , 'COLLECTE')" title="" data-toggle="tooltip" data-original-title="Delete"><i class="ti-trash"></i></a>
                        <a  v-if="parseInt(collecteData.id) === 3" href="javascript:vcode(0)" class="btn btn-circle btn-danger btn-xs" @click="HandleIdDelete(data.enquete?.id , 'HEBDOMADAIRE')" title="" data-toggle="tooltip" data-original-title="Delete"><i class="ti-trash"></i></a>
                        <a  v-if="parseInt(collecteData.id) === 4" href="javascript:vcode(0)" class="btn btn-circle btn-danger btn-xs" @click="HandleIdDelete(data.enquete?.id , 'JOURNALIER')" title="" data-toggle="tooltip" data-original-title="Delete"><i class="ti-trash"></i></a>
                        <a  v-if="parseInt(collecteData.id) === 8" href="javascript:vcode(0)" class="btn btn-circle btn-danger btn-xs" @click="HandleIdDelete(data.enquete?.id , 'TRANSFRONTALIER')" title="" data-toggle="tooltip" data-original-title="Delete"><i class="ti-trash"></i></a>
                    </div>   
					</td > 
               </tr>
           </tbody>
