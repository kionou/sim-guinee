...
<div class="box">
   <div class="box-header with-border d-flex justify-content-between align-items-center p-3">
     <h3 class="box-title mb-0">Liste des fiches enquêtes sur le marché <b style="color:red !important">{{nom }}</b>  </h3>
     ...
     <div class="navbar-custom-menu r-side">
        <ul class="nav navbar-nav justify-content-end">	
					...
					<div class="search-bx mx-5">
						<form>
							<div class="input-group">
							  <input type="text" class="form-control" placeholder="Search" aria-label="Recherchez..." aria-describedby="button-addon2" v-model="searchFicheGrossisteFiche" @input="filterByName">
			...
			<li class="h-40">
				<button type="button" class="waves-effect waves-circle btn btn-circle btn-primary mb-5" data-toggle="modal" data-target="#add-fiche-collecte" ><i class="mdi mdi-plus"></i></button>
			</li>
         ...
         <table id="example1" class="table table-bordered table-striped">
           ...
           <tbody v-else>
               <tr v-for="(data )  in paginatedItems" :key="data.id">
                   ...
                   <td style="width:100px !important ; color:red !important; font-weight:bolder !important" >{{ getYear( data?.date_enquete) }}</td>
                   
                   <td style="width: 120px;">
                    <div class="d-flex justify-content-evenly border-0">
                        <router-link :to="{ name: 'enquete-prix-par-marche-consommation', params: { id: data.id , nom:'CONSOMMATION' }}" class="btn btn-circle btn-success btn-xs" title="" @click="HandleData(data)"  data-original-title="view" ><i class="ti-eye"></i></router-link>
                        <a href="javascript:void(0)" class="btn btn-circle btn-info btn-xs" title="" @click="HandleIdUpdate(data.id)"  data-original-title="Update" data-toggle="modal" data-target="#update-famille"><i class="ti-marker-alt"></i></a>
   ...
   const d = new Date(date);
    ...
    async fetchMarches() {
      try {
        ...
        if (response.status === 200) {
			response.data.map(item => {
				this.MarchesOptions.push({
					label: item.nom_marche,
					value: item.id_marche
				});
});
        }
      } catch (error) {
   ...
   this.v$.step2.$touch();
    ...
    async HandleIdDelete(id) {
     ...
     if (result.isConfirmed) {
       ...
       filterByName() {
this.currentPage = 1;
if (this.searchFicheGrossisteFiche !== null) {
   const tt = this.searchFicheGrossisteFiche;
